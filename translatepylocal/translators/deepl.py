...
from translatepylocal.translators.base import BaseTranslator, BaseTranslateException
from translatepylocal.utils.annotations import Tuple, List
...
class JSONRPCRequest:
    ...
    def __init__(self, request: Request, timeout: int = 10) -> None:
        ...
        self.session = request
        self.last_access = 0
        self.timeout = timeout  # Ajoute un attribut pour gérer le timeout
...
class DeeplTranslate(BaseTranslator):
    ...
    def _translate(self, text: str, destination_language: str, source_language: str, formality: str = None,
        ...
        quality = ""
        if dictionary != "":
            if source_language.upper() not in dictionary.source_language.upper() or destination_language.upper() not in dictionary.target_language.upper():
                raise DeeplTranslateException(5011)
            limit = 10
            # check if word in the first column in dictionary is in the text to translate
        ...
        prev_sentences, _ = self._split_into_sentences(prev_paragraph, destination_language, source_language)
        next_sentences, _ = self._split_into_sentences(next_paragraph, destination_language, source_language)

        # building the a job per sentence
        jobs = self._build_jobs(sentences, quality, prev_sentences, next_sentences)
    ...
    def _build_jobs(self, sentences, quality="", prev_paragraph=[], next_paragraph=[]):
        """
        Builds a job for each sentence for DeepL, with context from surrounding paragraphs
        """
        jobs = []
        ...
        for index, sentence in enumerate(sentences):
            ...
            # Add context from previous paragraph if needed
            if prev_paragraph:
                # Calculate how many sentences we can take from prev_paragraph
                num_from_prev = max(0, 5 - index)
                before.extend(prev_paragraph[-num_from_prev:])

            # Add context from current paragraph
            if index > 0:
                before.extend(sentences[max(0, index - (5 - len(before))):index])

            # Add context from current or next paragraph for after context
            if index < len(sentences) - 1:
                after = [sentences[index + 1]]
            elif next_paragraph:
                after = [next_paragraph[0]]
