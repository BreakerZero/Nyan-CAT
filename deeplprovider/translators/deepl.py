...
from deeplprovider.models.languages import Language
from deeplprovider.utils.annotations import Tuple, Dict, List
...
class DeepL():
    ...
    def translate(self, text, destination_language, source_language=None, dictionary="test1,test1", formality=None, priority=1, quality="", compute_splitting=False) -> Union[Tuple[str, str], Tuple[None, None]]:
        """
        ...
        """
        countcomma = int(dictionary.count(',')/2)
        for specialChar in range(countcomma+1):
            dictionary = dictionary.replace(",", "\t", 1)
        ...
        try:
            ...
            """AFTERWORD\tMOT DE L'AUTEUR\nAsahina (Big)\tAsahina (Adulte)\nAsahina the Elder\tAsahina (Adulte)\nAsahina the Younger\tAsahina (Lycéenne)\nData Overmind\tEntité Psychique de Données\nhuh\thein\nof Nagato\tde Nagato\nthe Nagato\tla Nagato"""
    ...
    def language(self, text, priority=1, quality="", compute_splitting=False) -> Union[str, None]:
        ...
        try:
            ...
            params = {
                ...
                "commonJobParams":{
                    "formality": "informal",
                    "termbase":{ "dictionary": "Data Overmind\tEntité Psychique de Données"}},
                "timestamp": ts + (i_count - ts % i_count)
            }
