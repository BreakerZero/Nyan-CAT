...
def projecttextdocx(id):
	...
	if request.method == "GET":
		...
		#get project data
		(type, extension, last, files, keepstyle, complete, translatorsettings,
		 project) = get_project_data_for_get_method(id)
		if extension == "docx":
	...
	if request.method == "POST":
		...
		if extension == "docx":
			...
			if "ressource" in request.json:  #Demande ressouce fichier original
				...
				if idblock < len(
						TranslatedDocx.paragraphs):  #L'écriture du fichier de sortie s'écrivant au fur et à mesure
					if TranslatedDocx.paragraphs[
						idblock].text != text:  #Si le text est différent on enregistre ce nouveau texte
					...
					translation = TranslatorAPI.translate(translator, provider, settings, apikey, source, target,
					                                      formality, text,
					                                      formatedGlossary)  #on tente une traduction si on a rien dans la mémoire de traduction
